...
//  热门cell,这里的cell只有一行,服务器每次返回4个字符串,根据字符串的长度重新布局button的位置

#import "PISHotCell.h"

@interface PISHotCell()
...
{
    _hotDatas = hotDatas;
    //判断长度是否是4,开发中可以这样写 应该服务器返回几条数据就赋值多少,而不是固定的写死数据,万一服务器返回的数据有错误,会造成用户直接闪退的,有时在某些不是很重要的东西无法确定返回的是否正确,建议用@try @catch来处理.
    ...
    //算出间距
    CGFloat margin = (PISAppWidth - 40 - self.hotButton1.bounds.size.width - self.hotButton2.bounds.size.width - self.hotButton3.bounds.size.width - self.hotButton4.bounds.size.width) / 3;
...
{
    PISHotCell *cell = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([self class]) owner:nil options:nil] lastObject];
